...
METHODS_NOTE = (
    ...
    "groups with defined rates. Optional bootstrap intervals are stratified by "
    "group and use the displayed seed. Metrics computed with opsaudit "
...
.pass { color: #166534; font-weight: 700; }.fail { color: #b91c1c; font-weight: 700; }.review { color: #92400e; font-weight: 700; }
</style>
</head>
<body>
<h1>opsaudit report</h1>
<p>Generated: {{ generated }} | n={{ result.n_total }}</p>
{% if context %}<h2>Audit context</h2><table><thead><tr><th>field</th><th>value</th></tr></thead><tbody>{% for item in context %}<tr><td>{{ item.key }}</td><td>{{ item.value }}</td></tr>{% endfor %}</tbody></table>{% endif %}
...
{% if evidence %}<table><thead><tr><th>type</th><th>detail</th></tr></thead><tbody>{% for item in evidence %}<tr><td class="review">{{ item.code }}</td><td>{{ item.message }}</td></tr>{% endfor %}</tbody></table>{% else %}<p>No evidence-quality review conditions were recorded.</p>{% endif %}
{% if confidence_intervals %}<h3>Bootstrap confidence intervals</h3><p>95% stratified bootstrap intervals from {{ result.bootstrap_samples }} resamples.</p><table><thead><tr><th>metric</th><th>lower</th><th>upper</th></tr></thead><tbody>{% for interval in confidence_intervals %}<tr><td>{{ interval.check }}</td><td>{{ interval.lower }}</td><td>{{ interval.upper }}</td></tr>{% endfor %}</tbody></table>{% endif %}
...
def to_markdown(result: AuditResult) -> str:
    ...
    Example:
        ...
        True
    """
    lines = ["# opsaudit report", f"Generated: {_utc_timestamp()} | n={result.n_total}"]
    if result.context:
        lines.extend(["", "## Audit context", "| field | value |", "| --- | --- |"])
    ...
    if result.review_reasons:
        ...
        for reason in result.review_reasons:
            lines.append(
                f"| REVIEW: {_markdown_cell(str(reason['code']))} | {_markdown_cell(str(reason['message']))} |"
            )
    else:
        lines.append("No evidence-quality review conditions were recorded.")
    if result.confidence_intervals:
        lines.extend(
            [
                "",
                "### Bootstrap confidence intervals",
                f"95% stratified bootstrap intervals from {result.bootstrap_samples} resamples.",
                "| metric | lower | upper |",
                "| --- | ---: | ---: |",
            ]
        )
        for check, interval in result.confidence_intervals.items():
            lines.append(
                f"| {_markdown_cell(check)} | {_format_value(interval['lower'])} | {_format_value(interval['upper'])} |"
            )
...
def to_html(result: AuditResult) -> str:
    ...
    intervals = [
        {
            "check": check,
            "lower": _format_value(interval["lower"]),
            "upper": _format_value(interval["upper"]),
        }
        for check, interval in result.confidence_intervals.items()
    ]
    return Template(HTML_TEMPLATE, autoescape=True).render(
        ...
        result=result,
        context=[{"key": key, "value": value} for key, value in result.context.items()],
        groups=groups,
        flags=_display_flags(result),
        evidence=result.review_reasons,
...
def _markdown_cell(value: str) -> str:
    """Keep user-provided text within one safe Markdown table cell."""
